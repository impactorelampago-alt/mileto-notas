...
Dropdown no titlebar que lista as categorias (= sections do Ops), troca a ativa, cria, renomeia, exclui e compartilha. Botão-gatilho mostra bolinha de cor + label + chevron. A busca ignora caixa e acentos; Enter abre o primeiro resultado e a busca revela categorias mesmo dentro de grupo fechado. `counts` conta notas por categoria. Cada linha: acento esquerdo (emerald se ativa; verde contínuo se compartilhada comigo, com badge "Compartilhada"); ações no hover do dono: Compartilhar, Renomear e Excluir. Usuários com acesso ao Histórico podem marcar como Programa tanto categorias próprias quanto categorias realmente compartilhadas com eles; isso não libera nenhuma das ações estruturais exclusivas do dono.
