...
15. **AUDITORIA COMPLETA (jun/2026, v1.4.2) — 51 achados confirmados (0 crítico; 3 alto, 17 médio, 31 baixo/info), verificados por workflow adversarial.** Corrigidos nesta leva:
    - **Back (arquivos prontos, ⚠️ PENDENTES DE APLICAR — o banco self-hosted não estava acessível):**
      - `notas_tasks_update_policies.sql` **REESCRITO**: a 1ª versão tinha 2 bugs — (i) `ANY(notas_editable_creator_ids())` é **erro de sintaxe** (a função é `setof uuid`, exige `IN (SELECT ...)`) → provavelmente NUNCA aplicou; (ii) sem trava de coluna no WITH CHECK → editor podia mudar `status`/`assignee` e mover task de coluna no board do Ops. Agora as 2 policies CONGELAM status/creator/assignee (só title/description/priority), espelhando `tasks_update_shared_editor` do pacote 2.
      - `notas_audit_fixes_v142.sql` (novo): **(B)** `notas_can_complete_task` agora exige `permission='EDIT'` na via de note_shares (VIEW-only não conclui/move pro DONE no Ops). **(E)** `notas_reopen_task` ganhou autorização SIMÉTRICA ao concluir (DONO/assignee/cargo_edit + shares) — antes DONO concluía mas não reabria. **(D)** `notas_create_note_for` valida que `p_status` tem o prefixo de 37 chars do `p_owner` (não cria task na coluna de terceiro).
