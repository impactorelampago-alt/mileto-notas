...
14. **Flags de compartilhamento invertidas + UPDATE tasks faltando + auto-título quebrado (jun/2026, v1.4.2) — 3 bugs raiz no compartilhamento:** (a) `!notImpersonating` em `notes-store.loadNotes` (linha 314) estava **invertido** — `is_shared_with_me`/`shared_permission` nunca eram setados fora de impersonação, tornando TODAS as notas de categoria compartilhada somente-leitura no Editor (canEdit caia pro editableIds, que Barbara normalmente não tem); corrigido para `notImpersonating`. (b) Sem policy UPDATE em `tasks` para editor de categoria compartilhada, ao salvar conteúdo a nota atualizava mas a task ficava com `description` antiga do dono → `syncNotesFromTaskDescriptions` podia (clock-skew) sobrescrever o texto digitado; corrigido com 2 policies PERMISSIVE em `tasks`: `notas_tasks_update_shared_editor` (reusa `notas_category_shared_with_me(id)`) e `notas_tasks_update_nucleo_editor` (DONO + `notas_editable_creator_ids()`). (c) `semTitulo` em `Editor.tsx` não incluía `'Nova nota'` (título padrão de nota nova) nem `'Sem titulo'` (sem acento, saída do RPC) → auto-título nunca atualizava notas recém-criadas; corrigido em 2 lugares (handleChange 600ms e note-switch). **Back:** arquivo `supabase/migrations/notas_tasks_update_policies.sql`. **Status: ✅ policies APLICADAS na VPS (jun/2026).** **Front:** só o Notas (notes-store.ts + Editor.tsx).
